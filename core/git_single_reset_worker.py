...
class GitSingleResetWorker(threading.Thread):
    def __init__(self, repo_obj, relative_file_path, on_finished_callback):
        super().__init__()
        self.repo = repo_obj
    ...
    def run(self):
        try:
            try:
                self.repo.git.reset('HEAD', self.rel_path)
            except Exception:
                pass

            self.repo.git.checkout(self.rel_path, force=True)
            wx.CallAfter(self.on_finished, True, self.rel_path, "")
        except Exception as e:
            wx.CallAfter(self.on_finished, False, self.rel_path, str(e))
