...
Spawn автоматически определяет кодировку файлов при их открытии.

Для исходных файлов Pawn (`.pwn` и `.inc`) можно явно указать кодировку с помощью директивы encoding в начале файла:

```pawn
// coding: cp1251
```

Если такая директива присутствует, Spawn будет использовать указанную кодировку при открытии и сохранении файла.

Если директива кодировки отсутствует, Spawn попытается определить кодировку файла автоматически.

#### Повторное открытие файлов с другой кодировкой

В некоторых случаях автоматическое определение может неправильно идентифицировать кодировку файла.

Если файл отображается некорректно, его можно открыть заново, используя другую кодировку:

`Edit → Encoding → Reopen`

*Эта операция не изменяет файл на диске и влияет только на то, как файл отображается в редакторе.*
...
* Windows-1257 (Прибалтийский)

#### Кодировка Pawn по умолчанию

Вы можете настроить кодировку по умолчанию, используемую для вновь создаваемых исходных файлов Pawn.

При создании нового файла с расширением `.pwn` или `.inc` Spawn может автоматически вставлять директиву кодировки в соответствии с выбранной кодировкой по умолчанию.
